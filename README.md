...
- **多种回复形式**：一个关键词可配置多个回复，机器人会随机选择一条发送
- **多种回复类型**：可同时回复 文字+图片+视频+文件
- **多种回复规则**：设置别名、是否唤醒、正则匹配、定时触发、黑白名单、频率控制
- **WebUI管理**：提供图形化界面管理关键词配置，支持在线预览和编辑
- **灵活配置**：对每个关键词进行模块化储存，便于导入、导出数据
...
- `cron_config`：定时任务配置对象
  - `cron_expression`：Cron表达式，用于设置定时任务的执行时间
  - `whitelist`：定时发送白名单，格式为 `@群号`或 `#QQ号`
  - `blacklist`：定时发送黑名单，格式为 `@群号`或 `#QQ号`
- `enabled`：是否启用关键词（默认：true）
